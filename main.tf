...
data "template_file" "ipv4_addrs" {
  ...
  vars {
    etcd_id = "${element(data.template_file.instances_ids.*.rendered, count.index)}"
  }
}
...
module "userdata" {
  ...
  ipv4_addrs           = ["${coalescelist(flatten(openstack_networking_port_v2.port_etcd.*.all_fixed_ips), data.template_file.public_ipv4_addrs.*.rendered)}"]
...
data "template_file" "instances_ids" {
  ...
  vars {
    id               = "${element(coalescelist(openstack_compute_instance_v2.singlenet_etcd.*.id, openstack_compute_instance_v2.multinet_etcd.*.id), count.index)}"
